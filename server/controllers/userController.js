...
export const register = async (req, res) => {
    ...
    const tokenPayload = generateTokenPayload({ userId: newUser._id, firstName: newUser.firstName, lastName: newUser.lastName });
    const accessToken = await jwt.sign(tokenPayload, ACCESS_TOKEN_SECRET, { expiresIn: ACCESS_TOKEN_EXPIRY });
    const refreshToken = await jwt.sign(tokenPayload, REFRESH_TOKEN_SECRET, { expiresIn: REFRESH_TOKEN_EXPIRY });
    await Token.create({ refreshToken, user: newUser._id });
    res.status(201)
        .cookie("refresh_token", refreshToken, { httpOnly: true, expires: COOKIE_EXPIRY })
        .json(successRes("Registration successful", res.statusCode, { accessToken, expiresIn: ACCESS_TOKEN_EXPIRY }));
};
...
export const login = async (req, res) => {
    ...
    const tokenPayload = generateTokenPayload({ userId: foundUser._id, firstName: foundUser.firstName, lastName: foundUser.lastName });
    const accessToken = await jwt.sign(tokenPayload, ACCESS_TOKEN_SECRET, { expiresIn: ACCESS_TOKEN_EXPIRY });
    ...
    res.status(200)
        .cookie("refresh_token", refreshToken, { httpOnly: true, expires: COOKIE_EXPIRY })
        .json(successRes("Login successful", res.statusCode, { accessToken, expiresIn: ACCESS_TOKEN_EXPIRY }));
};
...
export const generateAccessToken = async (req, res) => {
    ...
    res.status(201).json(successRes("Success", res.statusCode, { accessToken, expiresIn: ACCESS_TOKEN_EXPIRY }));
};
...
export const logout = async (req, res) => {
    ...
    res.status(200).clearCookie("refresh_token").json(successRes("Success", res.statusCode));
};
